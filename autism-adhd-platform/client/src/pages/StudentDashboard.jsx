...
import { LogOut, ChevronRight, Search, X, Lock, ShieldAlert } from 'lucide-react';
...
const ALL_CARDS = [
  ...
  },
  {
    ...
    emoji:   '⭐',
    title:   'My Stars',
    desc:    'See all the stars you earned!',
    tags:    ['Stars', 'Wins'],
    btnTxt:  'See Stars!',
  },
];
...
export default function StudentDashboard() {
  ...
  const verifyParentPin = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setIsParentUnlocked(true); setShowParentModal(false);
        setParentPin(''); setPasswordError('');
      } else {
  ...
  const toggleParentView = () => {
    if (isParentUnlocked) setIsParentUnlocked(false);
      ...
      <nav className="dashboard-nav">
        ...
        <div className="nav-right">
          <button onClick={toggleParentView}
            className={`nav-btn nav-btn-parent${isParentUnlocked ? ' unlocked' : ''}`}>
            {isParentUnlocked ? <ShieldAlert size={15}/> : <Lock size={15}/>}
            {isParentUnlocked ? 'Lock View' : 'Parent'}
          </button>
