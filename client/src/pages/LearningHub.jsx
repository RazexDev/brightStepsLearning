...
import { Volume2, FileText, ExternalLink, LogOut, Sparkles, Download, RefreshCcw, Lock, X, Eye, EyeOff, PlayCircle } from 'lucide-react';
...
export default function LearningHub() {
  ...
  const logProgressAndRefresh = async (activityTitle, type) => {
    // Silently track resource view for the Parent Report
    try {
      ...
      if (studentId) {
        await fetch('/api/resources/track', {
          ...
          body: JSON.stringify({ studentId, resourceName: activityTitle, resourceType: type }),
        });
      }
    } catch { /* silent — tracking should never block the UX */ }
    ...
    <div className="student-container" style={{ backgroundColor: '#fdfbfb', minHeight: '100vh', padding: '40px', position: 'relative' }}>
      ...
      <div style={{ marginBottom: '24px', position: 'relative' }}>
        ...
        <input
          ...
          placeholder="Search by title, format (video, pdf), or level..."
          style={{
            width: '100%',
          ...
          onFocus={(e) => {
            ...
            e.target.style.boxShadow = '0 4px 24px rgba(108,92,231,0.22)';
          }}
          onBlur={(e) => {
            e.target.style.borderColor = 'rgba(108, 92, 231, 0.25)';
            e.target.style.boxShadow = '0 4px 20px rgba(108,92,231,0.10)';
          }}
        />
        {searchQuery && (
          <button
            ...
            style={{
              ...
              background: 'rgba(108,92,231,0.12)', border: 'none', borderRadius: '50%',
              width: 30, height: 30, cursor: 'pointer', color: '#6C5CE7',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      {(() => {
        ...
        const filteredResources = q
          ? resources.filter((res) => {
              ...
              const levelStr    = String(res.requiredLevel ?? 0);
          ...
          <div className="student-grid">
                ...
                {res.type === 'offline' ? (
                  ...
                  res.type === 'video' ? (
                    <button 
                      ...
                      onClick={() => {
                        logProgressAndRefresh(res.title, 'Video');
                    ...
                    <a 
                      ...
                      onClick={() => logProgressAndRefresh(res.title, res.type === 'pdf' ? 'PDF Document' : 'Link')}
                    >
            ...
            {filteredResources.length === 0 && (
              <h3 style={{ textAlign: 'center', color: '#b2bec3', width: '100%' }}>
                {searchQuery.trim()
                  ? `😕 No resources match "${searchQuery.trim()}". Try a different keyword!`
