...
const CATEGORY_META = {
  morning: { color: "#f28b60", bg: "#fde8d8", label: "Morning",  badge: "DAILY MUST!",    badgeBg: "#FEF08A", badgeColor: "#92400E" },
  school:  { color: "#4ab3e8", bg: "#cce8ff", label: "School",   badge: "SCHOOL TIME!",  badgeBg: "#BAE6FD", badgeColor: "#0C4A6E" },
  bedtime: { color: "#b48be0", bg: "#e8e0ff", label: "Bedtime",  badge: "SWEET DREAMS!", badgeBg: "#DDD6FE", badgeColor: "#5B21B6" },
  study:   { color: "#3dbf9a", bg: "#d4f5ec", label: "Study",    badge: "FOCUS TIME!",   badgeBg: "#A7F3D0", badgeColor: "#064E3B" },
  evening: { color: "#e6b800", bg: "#fff7cc", label: "Evening",  badge: "EVENING!",      badgeBg: "#FEF08A", badgeColor: "#78350F" },
  custom:  { color: "#6b7c75", bg: "#eaf9f5", label: "Custom",   badge: null },
};

const TYPE_META = {
  adhd:    { label: "ADHD",    color: "#4ab3e8" },
  autism:  { label: "Autism",  color: "#b48be0" },
  general: { label: "General", color: "#3dbf9a" },
};
    ...
    <div className="rd-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="rd-modal rd-form-modal">
        ...
        <div className="rd-form-header">
          ...
          <div>
            <h2 className="rd-form-title">Auto Routine Generator</h2>
...
function RoutineCard({ routine, onEdit, onDelete, viewMode }) {
  ...
  return (
    <div
      className={`rd-card ${collapsed ? "rd-card-collapsed" : ""}`}
    ...
    >
      ...
      <div className="rd-card-body">
        <div className="rd-card-top-row">
          <h3 className="rd-card-title">{routine.title}</h3>
          <span className="rd-type-badge" style={{ background: type.color + "22", color: type.color }}>
            {type.label}
          </span>
        </div>
        ...
        {!collapsed && (
          <>
            <div className="rd-chip-row" style={{ marginTop: 6 }}>
              <span className="rd-chip-tiny" style={{ background: cat.bg, color: cat.color }}>{cat.label}</span>
              {totalMins > 0 && (
                <span className="rd-chip-tiny rd-chip-clock"><Clock size={10} /> {totalMins} min</span>
              )}
              {routine.rewards?.badgesEarned?.map((b, i) => (
                <span key={i} className="rd-chip-tiny" style={{ background: "#fff7cc", color: "#92400E" }}>{b}</span>
              ))}
            </div>
            <div className="rd-task-list" style={{ marginTop: 10 }}>
              ...
              {!expanded && routine.tasks?.length > 3 && (
                <button className="rd-show-more" onClick={() => setExpanded(true)}>
                  +{routine.tasks.length - 3} more <ChevronDown size={12} />
                </button>
              )}
              {expanded && routine.tasks?.length > 3 && (
            ...
            <div className="rd-task-count-row">
              📋 {routine.tasks?.length || 0} task{routine.tasks?.length !== 1 ? "s" : ""}
              &nbsp;·&nbsp; ⭐ {routine.rewards?.starsEarned || 0} stars
            </div>
          </>
        )}
      </div>

      {/* Right: badge + start btn + controls */}
      <div className="rd-card-right">
        {cat.badge && (
          <span className="rd-cat-badge" style={{ background: cat.badgeBg, color: cat.badgeColor }}>
            {cat.badge}
          </span>
        )}
        <ProgressRing pct={routine.progress || 0} size={44} stroke={4.5} color={cat.color} />
        <button
          className="rd-start-btn"
          style={{ background: cat.color, boxShadow: `0 6px 0 ${cat.color}aa` }}
          title="Start routine"
        >
          ▶<br /><span style={{ fontSize: "0.6rem", fontFamily: "'Nunito',sans-serif", fontWeight: 800 }}>Start</span>
        </button>
        <div className="rd-card-controls" style={{ justifyContent: "center" }}>
          ...
          <button className="rd-ctrl-btn" onClick={() => onEdit(routine)} title="Edit">
            <Pencil size={13} />
          </button>
          <button className="rd-ctrl-btn danger" onClick={() => onDelete(routine)} title="Delete">
            <Trash2 size={13} />
          </button>
        </div>
      </div>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .rd-hub-title {
          ...
          background: linear-gradient(135deg, #f28b60 0%, #4ab3e8 50%, #3dbf9a 100%);
          -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
        }

        .rd-tw { display: inline-block; }
        .rd-tw-mint  { color: #3dbf9a; }
        .rd-tw-sky   { color: #4ab3e8; }

        .rd-hub-subtitle { font-size: 1.15rem; color: var(--text-mid,#6b7c75); font-weight: 700; margin-bottom: 18px; }
        ...
        /* ── STATS GRID ── */
        .rd-stats-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(190px, 1fr)); gap: 16px; }

        .rd-stat-card {
          background: var(--card-bg, white);
          border-radius: 24px; padding: 24px 18px;
          box-shadow: 0 8px 0 var(--card-shadow, rgba(0,0,0,0.06)), 0 16px 32px rgba(0,0,0,0.07);
          display: flex; flex-direction: column; align-items: center; gap: 8px; text-align: center;
          transition: transform 0.22s ease, box-shadow 0.22s ease;
          border: none;
        }
        .rd-stat-card:hover { transform: translateY(-6px); box-shadow: 0 14px 0 var(--card-shadow, rgba(0,0,0,0.08)), 0 24px 40px rgba(0,0,0,0.1); }
        ...
        .rd-stat-num { font-family: 'Fredoka One', cursive; font-size: 2.4rem; color: var(--accent-color, #3dbf9a); line-height: 1; }
        .rd-stat-label { font-size: 0.88rem; font-weight: 800; color: var(--text-mid,#6b7c75); letter-spacing: 0.3px; }
        ...
        /* ── ROUTINE CARD (horizontal tile) ── */
        .rd-card {
          background: white;
          border-radius: 24px;
          border-left: 7px solid var(--cat-color, #3dbf9a);
          box-shadow: 0 6px 0 rgba(0,0,0,0.06), 0 14px 36px rgba(0,0,0,0.08);
        ...
        .rd-card:hover { transform: translateY(-5px); box-shadow: 0 10px 0 rgba(0,0,0,0.07), 0 22px 44px rgba(0,0,0,0.11); }
        .rd-card-collapsed { padding-bottom: 16px; }
        .rd-template-card { border-left: 6px solid var(--cat-color, #3dbf9a); }
        ...
        .rd-card-top-row { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin-bottom: 2px; }
        .rd-card-title { font-family: 'Fredoka One', cursive; font-size: 1.35rem; color: #2d3a35; line-height: 1.2; }
        .rd-type-badge {
        ...
        .rd-task-count-row {
          ...
          color: var(--text-mid,#6b7c75); margin-top: 8px;
        }

        .rd-chip-row { display: flex; gap: 5px; flex-wrap: wrap; }
        .rd-chip-tiny {
          display: inline-flex; align-items: center; gap: 3px;
          padding: 3px 11px; border-radius: 999px;
          font-size: 0.74rem; font-weight: 800;
        }
        .rd-chip-clock { background: #d4f5ec !important; color: #3dbf9a !important; }
        ...
        /* Special category badge e.g. DAILY MUST! */
        .rd-cat-badge {
          font-family: 'Fredoka One', cursive;
        ...
        .rd-start-btn {
          ...
          color: white; font-size: 1.1rem; font-family: 'Fredoka One', cursive;
          display: flex; flex-direction: column; align-items: center; justify-content: center;
          line-height: 1.1;
          transition: transform 0.18s cubic-bezier(.34,1.56,.64,1), box-shadow 0.18s;
        }
        .rd-start-btn:hover { transform: scale(1.12); }
        .rd-start-btn:active { transform: scale(0.95); }

        /* Window controls */
        .rd-card-controls { display: flex; gap: 4px; flex-wrap: wrap; }

        .rd-ctrl-btn {
          width: 28px; height: 28px; border-radius: 50%;
          background: #eaf9f5; border: none; cursor: pointer;
          display: flex; align-items: center; justify-content: center;
          color: #6b7c75;
          transition: background 0.15s ease, transform 0.15s ease, color 0.15s ease;
        }
        .rd-ctrl-btn:hover { background: #d4f5ec; color: #3dbf9a; transform: scale(1.1); }
        .rd-ctrl-btn.danger:hover { background: #fde8e8; color: #e07070; }
        .rd-ctrl-btn.success:hover { background: #d4f5ec; color: #3dbf9a; }
        ...
        .rd-progress-label { font-family: 'Fredoka One', cursive; font-size: 0.95rem; color: #2d3a35; }
        .rd-star-count { font-size: 0.82rem; font-weight: 700; color: #6b7c75; }
        .rd-badges-inline { font-size: 1rem; }

        /* Task list */
        .rd-task-list { display: flex; flex-direction: column; gap: 6px; }
        .rd-task-row { display: flex; align-items: center; gap: 8px; padding: 4px 0; }
        .rd-task-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
        .rd-task-label { flex: 1; font-size: 0.88rem; font-weight: 700; color: #2d3a35; }
        .rd-task-mins { font-size: 0.76rem; font-weight: 700; color: #6b7c75; background: #eaf9f5; padding: 1px 8px; border-radius: 999px; }

        .rd-show-more {
          display: inline-flex; align-items: center; gap: 4px;
          background: none; border: none; cursor: pointer;
          font-family: 'Nunito', sans-serif; font-weight: 800; font-size: 0.82rem;
          color: #3dbf9a; padding: 4px 0;
        }
        .rd-show-more:hover { color: #4ab3e8; }
          ...
          <div className="rd-section">
            ...
            {!sectionsCollapsed.stats && summary && (
              <div className="rd-stats-grid">
                {[
                  { label: "Assigned",   val: summary.totalAssigned || 0,             emoji: "📋", accent: "#4ab3e8", gradient: "linear-gradient(145deg,#cce8ff,#8dd4f8)" },
                  { label: "Completed",  val: summary.completedCount || 0,            emoji: "✅", accent: "#3dbf9a", gradient: "linear-gradient(145deg,#d4f5ec,#7decd4)" },
                  { label: "Completion", val: `${summary.completionPercentage || 0}%`, emoji: "📈", accent: "#f28b60", gradient: "linear-gradient(145deg,#fde8d8,#ffb899)" },
                  { label: "Stars",      val: summary.totalStars || 0,                emoji: "⭐", accent: "#e6b800", gradient: "linear-gradient(145deg,#fff7cc,#ffe566)" },
                ].map((s) => (
                  <div key={s.label} className="rd-stat-card"
                    style={{ "--accent-color": s.accent, "--card-bg": s.gradient, "--card-shadow": s.accent + "55" }}>
                    <div className="rd-stat-icon" style={{ background: "rgba(255,255,255,0.55)", backdropFilter: "blur(4px)" }}>{s.emoji}</div>
