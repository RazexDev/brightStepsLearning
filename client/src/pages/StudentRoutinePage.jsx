...
/* ─── ROUTINE CARD ─── */
function RoutineCard({ routine, started, onStart, onToggleTask, showMotivation, isDone, focusMode, calmMode, viewMode }) {
  const [collapsed, setCollapsed] = useState(false);
  const [showAllTasks, setShowAllTasks] = useState(false);

  const cat = CAT[routine.category] || CAT.custom;
  const typeC = TYPE_COLORS[routine.type] || TYPE_COLORS.general;
  // In calm mode, dampen the card's visual intensity
  const cardStyle = calmMode
    ? { "--cat-color": cat.color, "--cat-bg": cat.bg, opacity: 0.93, filter: "saturate(0.75)" }
...
export default function StudentRoutinePage() {
  ...
  return (
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sr-mode-btn {
          ...
          padding: 11px 22px; border-radius: 999px; border: 2.5px solid transparent;
          background: white; font-family: 'Nunito', sans-serif; font-weight: 800; font-size: 0.9rem;
          color: #6b7c75; cursor: pointer;
          box-shadow: 0 5px 0 rgba(0,0,0,0.06), 0 8px 20px rgba(0,0,0,0.07);
          transition: all 0.22s cubic-bezier(0.34,1.56,0.64,1);
        }
        .sr-mode-btn:hover { transform: translateY(-3px); box-shadow: 0 8px 0 rgba(0,0,0,0.07), 0 14px 28px rgba(0,0,0,0.1); }
        /* Focus mode = orange/amber gradient */
        .sr-mode-btn.active { background: linear-gradient(135deg, #f28b60, #f5c842); color: white; border-color: transparent; box-shadow: 0 5px 0 #c8881a, 0 10px 24px rgba(242,139,96,0.35); }
        /* Calm mode = lavender/purple gradient */
        .sr-mode-btn.calm.active { background: linear-gradient(135deg, #b48be0, #7c5cbf); color: white; border-color: transparent; box-shadow: 0 5px 0 #6B52B0, 0 10px 24px rgba(180,139,224,0.4); }
        ...
        .sr-section-title {
          font-family: 'Fredoka One', cursive; font-size: 1.35rem; color: #2d3a35;
          flex: 1; display: flex; align-items: center; gap: 8px;
        }
        /* Per-section accent colors */
        .sr-section-title.recommended { color: #6040c0; }
        .sr-section-title.active      { color: #f28b60; }
        .sr-section-title.done        { color: #3dbf9a; }
        ...
        .sr-section-toggle { width: 32px; height: 32px; border-radius: 50%; background: #d4f5ec; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #3dbf9a; transition: all 0.2s; }
        .sr-section-toggle:hover { background: #3dbf9a; color: white; }

        /* Calm-mode page tint */
        .sr-page-calm { background-color: #F0EEFF !important; }
        .sr-page-calm .sr-hero { background: linear-gradient(135deg, #EDEAFA 0%, #DDD6FE 55%, #E8E0FF 100%) !important; }
        ...
        /* ROUTINE CARD */
        .sr-card {
          background: linear-gradient(160deg, var(--cat-bg, #E0F7F3) 0%, #fff 55%);
          border-radius: var(--r-lg);
          border-left: 7px solid var(--cat-color, #3DB5A0);
          box-shadow: var(--sh-card);
          padding: 22px;
          transition: transform 0.3s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.3s ease;
          animation: cardIn 0.4s cubic-bezier(0.34,1.56,0.64,1) both;
        }
        .sr-card:hover { transform: translateY(-8px) scale(1.01); box-shadow: 0 18px 0 rgba(30,16,7,0.06), 0 32px 64px rgba(30,16,7,0.13); }
        .sr-card-done {
          background: linear-gradient(160deg, #FEF4CC 0%, #fffdf5 55%) !important;
          border-left-color: #C8881A !important;
          opacity: 0.92;
        }
        ...
        .sr-card-head { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 14px; }
        .sr-card-emoji {
          width: 52px; height: 52px; border-radius: 18px;
          display: flex; align-items: center; justify-content: center;
          font-size: 1.8rem; flex-shrink: 0;
          background: var(--cat-bg, #E0F7F3);
          box-shadow: 0 4px 0 var(--cat-color, #3DB5A0);
        }
        .sr-card-head-info { flex: 1; min-width: 0; }
        .sr-card-title {
          font-family: 'Fredoka One', cursive; font-size: 1.2rem;
          color: var(--cat-color, #1E8C78); line-height: 1.2; margin-bottom: 5px;
        }
        ...
        .sr-progress-row { display: flex; align-items: center; gap: 14px; margin-bottom: 14px; }
        .sr-progress-info { display: flex; flex-direction: column; gap: 4px; }
        .sr-progress-label {
          font-family: 'Fredoka One', cursive; font-size: 1.05rem;
          color: var(--cat-color, #1E8C78);
        }
        ...
        .sr-task-item {
          ...
          border: 2px solid var(--cat-color, #3DB5A0);
          border-opacity: 0.15;
        ...
        .sr-task-item:not(.locked):hover {
          background: white;
          border-color: var(--cat-color, #3DB5A0);
          transform: scale(1.02) translateX(6px);
          box-shadow: 0 4px 16px rgba(0,0,0,0.07);
        }
        .sr-task-item.done {
          opacity: 0.65;
          background: linear-gradient(90deg, var(--cat-bg, #E0F7F3), rgba(255,255,255,0.4));
        }
        .sr-task-item.done .sr-task-label { text-decoration: line-through; color: var(--ink-soft); }
        .sr-task-item.locked { cursor: not-allowed; opacity: 0.4; }
        ...
        .sr-status-chip { display: inline-flex; align-items: center; gap: 5px; padding: 7px 16px; border-radius: 999px; font-size: 0.82rem; font-weight: 800; }
        .sr-status-chip.done {
          background: linear-gradient(135deg, #f5c842, #f28b60);
          color: white;
          box-shadow: 0 4px 0 #c8881a, 0 8px 16px rgba(242,139,96,0.3);
        }
        .sr-done-chip {
        ...
        /* LIST ITEM */
        .sr-list-item {
          background: linear-gradient(135deg, var(--cat-bg, #E0F7F3) 0%, #fff 60%);
          border-radius: 20px;
          border-left: 6px solid var(--cat-color, #3DB5A0);
          box-shadow: 0 6px 0 rgba(30,16,7,0.05), 0 12px 28px rgba(30,16,7,0.07);
          padding: 14px 18px; display: flex; align-items: center; gap: 14px;
          animation: cardIn 0.3s ease both;
          transition: transform 0.2s ease, box-shadow 0.2s ease;
        }
        .sr-list-item:hover { transform: translateY(-4px); box-shadow: 0 10px 0 rgba(30,16,7,0.06), 0 20px 36px rgba(30,16,7,0.1); }
      ...
      <div className={`sr-page${calmMode ? " sr-page-calm" : ""}`}>
            ...
            <>
              ...
              {recommendedRoutines.length > 0 && (
                <div className="sr-section sr-recommended-section">
                  <div className="sr-section-head">
                    <h2 className="sr-section-title recommended">
                    ...
                    {recommendedRoutines.map((r, i) => (
                      <div key={r._id} style={{ animationDelay: `${i * 0.08}s` }}>
                        <RoutineCard routine={r} started={startedRoutineId === r._id}
                          ...
                          showMotivation={showMotivation} focusMode={focusMode} calmMode={calmMode} viewMode={viewMode} />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeRoutines.length > 0 && (
                <div className="sr-section">
                  <div className="sr-section-head">
                    <h2 className="sr-section-title active">
                      🚀 Active Routines <span className="sr-count-chip" style={{ background: "#fde8d8", color: "#f28b60" }}>{activeRoutines.length}</span>
                    </h2>
                  </div>
                  <div className={viewMode === "grid" ? "sr-cards-grid" : "sr-cards-list"}>
              ...
              {doneRoutines.length > 0 && (
                <div className="sr-section">
                  <div className="sr-section-head">
                    <h2 className="sr-section-title done">
                      ✅ Completed <span className="sr-count-chip" style={{ background: "#d4f5ec", color: "#3dbf9a" }}>{doneRoutines.length}</span>
                    </h2>
                      ...
                      {doneRoutines.map((r, i) => (
                        <div key={r._id} style={{ animationDelay: `${i * 0.05}s` }}>
                          <RoutineCard routine={r} started={false} onStart={() => {}}
                            ...
                            isDone focusMode={false} calmMode={calmMode} viewMode={viewMode} />
                        </div>
                      ))}
                    </div>
                  )}
